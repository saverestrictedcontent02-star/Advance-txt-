...
AUTH_CHANNELS = [-1003651358527]  # Add more channel IDs here
...
try:
    for i in range(count - 1, len(links)):
        ...
        url = "https://" + V
            # ================= VISIONIAS =================
            if "visionias" in url:  # Level 3
                async with ClientSession() as session:  # Level 4 - FIXED: Now inside async function!
            ...
            # Download and send based on file type
            ...
            if "drive" in url:
                ...
                await handle_pdf_download(url, name, cc1, m, bot, count)

            elif any(ext in url.lower() for ext in [".jpg", ".jpeg", ".png"]):
                await handle_image_download(url, name, cimg, m, bot, count)

            elif ".zip" in url:
                await handle_zip_download(url, name, cczip, m, bot, count)

            else:  # Video download
                await handle_video_download(url, name, cc, m, bot, count, helper, res, len(links), thumb)

        except Exception as e:  # INNER EXCEPT - catches per-link errors
...
except Exception as e:  # OUTER EXCEPT - FIXED: Now at correct indentation (Level 0)
    await m.reply_text(e)

# ================= SUMMARY MESSAGES (Outside loop) =================
